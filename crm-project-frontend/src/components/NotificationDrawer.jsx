...
export default function NotificationDrawer({ isOpen, onClose, onUnreadCountChange }) {
  ...
  const fetchDynamicNotifications = useCallback(async () => {
    ...
    try {
      ...
      // 4. AMC CONTRACT NOTIFICATIONS
      amcs.slice(0, 5).forEach((amc) => {
      ...
      projectsList.slice(0, 5).forEach((prj) => {
      ...
      // 6. ACCOUNTS & STAFF NOTIFICATIONS
      staffList.slice(0, 5).forEach((st) => {
      ...
      // 7. CUSTOM DISPATCHED NOTIFICATIONS (Password Requests, System Alerts)
  ...
  // Listen to custom window events & storage changes for instant real-time sync without refresh
  useEffect(() => {
            ...
            {loading ? (
              ...
              filteredNotifications.map((item) => (
                <div
                  ...
                  className={`p-3.5 rounded-xl border-2 transition-all duration-200 relative group bg-white cursor-pointer hover:shadow-md hover:-translate-y-0.5 ${!item.read
